...
class HFETArchitecture(ModelArchitecture):
    name = "HFET"
    ...
    def parse_weights(self, state_dict: Dict[str, np.ndarray]) -> None:
        ...
        candidate_roots = [
            os.path.join(project_root, "checkpoints", "HFET"),
            os.path.join(project_root, "checkpoint", "HFET"),
        ]
        ckpt_dir = next((c for c in candidate_roots if os.path.isdir(c)), None)
        if ckpt_dir is None:
            raise ExportError(f"HFET checkpoint directory not found under: {candidate_roots}")

        idvd_dir = os.path.join(ckpt_dir, "IdVd")
        idvg_dir = os.path.join(ckpt_dir, "IdVg")
        ...
        self.idvd_ae = torch.load(
            os.path.join(idvd_dir, "idvd_3_curves_linear_scale_hch_hAlGaN_filter.pth"),
            map_location="cpu",
        )
        self.idvd_poly_model = torch.load(
            os.path.join(idvd_dir, "idvd_3_curves_linear_scale_poly_hch_hAlGaN_filter.pth"),
            map_location="cpu",
        )
        self.idvd_scaler_x = joblib.load(os.path.join(idvd_dir, "scaler_x.pkl"))
        self.idvd_scaler_iv = joblib.load(os.path.join(idvd_dir, "scaler_iv.pkl"))
        self.idvd_scaler_ls = joblib.load(os.path.join(idvd_dir, "scaler_ls.pkl"))
        ...
        self.idvg_ae = torch.load(
            os.path.join(idvg_dir, "idvg_2_curves_log_linear_scale.pth"),
            map_location="cpu",
        )
        self.idvg_poly_model = torch.load(
            os.path.join(idvg_dir, "poly_regression_model.pth"),
            map_location="cpu",
        )
        self.idvg_scaler_x = joblib.load(os.path.join(idvg_dir, "scaler_x.pkl"))
        self.idvg_scaler_iv = joblib.load(os.path.join(idvg_dir, "scaler_iv_linear.pkl"))
        self.idvg_scaler_ls = joblib.load(os.path.join(idvg_dir, "scaler_ls.pkl"))
    ...
    def emit_model(self, module_name: str) -> str:
        ...
        lines: List[str] = [
            "// Automatically generated Verilog-A HFET drain-current model",
            ...
            "  electrical d, g, s;",
            "",
            "  // Geometry parameters in um, matching the ML backend inputs.",
            "  parameter real Lsg = 1.0;",
            "  parameter real Lgd = 6.0;",
            "  parameter real Lg = 1.4;",
            "  parameter real hpas = 0.2;",
            "  parameter real hAlGaN = 0.02;",
            "  parameter real hch = 0.01;",
            "  parameter real hg = 0.11;",
        ...
        lines.extend(
            [
                "",
                "      safe_halg = (hAlGaN > 1.0e-12) ? hAlGaN : 1.0e-12;",
                "      if(hch > 0.019 * hch / safe_halg - 0.002) begin",
                "        input_x[0] = Lsg * idvd_sx_mul[0] + idvd_sx_add[0];",
                "        input_x[1] = Lgd * idvd_sx_mul[1] + idvd_sx_add[1];",
                "        input_x[2] = Lg * idvd_sx_mul[2] + idvd_sx_add[2];",
                "        input_x[3] = hpas * idvd_sx_mul[3] + idvd_sx_add[3];",
                "        input_x[4] = hAlGaN * idvd_sx_mul[4] + idvd_sx_add[4];",
                "        input_x[5] = hch * idvd_sx_mul[5] + idvd_sx_add[5];",
                "        input_x[6] = hg * idvd_sx_mul[6] + idvd_sx_add[6];",
            ]
        )
        ...
        lines.extend(
            [
                ...
                "        end",
                "      end else begin",
                f"        for(i=0; i<{idvd_out_len}; i=i+1) idvd_out[i] = 0.0;",
                "      end",
                "",
                "      input_x[0] = Lsg * idvg_sx_mul[0] + idvg_sx_add[0];",
                "      input_x[1] = Lgd * idvg_sx_mul[1] + idvg_sx_add[1];",
                "      input_x[2] = Lg * idvg_sx_mul[2] + idvg_sx_add[2];",
                "      input_x[3] = hpas * idvg_sx_mul[3] + idvg_sx_add[3];",
                "      input_x[4] = hAlGaN * idvg_sx_mul[4] + idvg_sx_add[4];",
                "      input_x[5] = hch * idvg_sx_mul[5] + idvg_sx_add[5];",
                "      input_x[6] = hg * idvg_sx_mul[6] + idvg_sx_add[6];",
            ]
        )
        ...
        lines.extend(
            [
                ...
                "",
                "    // Id-Vd surface: 3 Vg anchors x 300 Vd samples",
                "    if(val_vg <= 1.0) begin",
                "      idvd_vg0_idx = 0;",
                "      idvd_vg1_idx = 1;",
                "      idvd_vg_frac = 0.0;",
                "      idvd_excess = 1.0 - val_vg;",
                "    end else if(val_vg >= 5.0) begin",
                "      idvd_vg0_idx = 1;",
                "      idvd_vg1_idx = 2;",
                "      idvd_vg_frac = 1.0;",
                "      idvd_excess = val_vg - 5.0;",
                "    end else begin",
                "      idvd_vg_pos = (val_vg - 1.0) / 2.0;",
                "      idvd_vg0_idx = $rtoi(idvd_vg_pos);",
                "      if(idvd_vg0_idx > 1) idvd_vg0_idx = 1;",
                "      idvd_vg1_idx = idvd_vg0_idx + 1;",
                "      idvd_vg_frac = idvd_vg_pos - idvd_vg0_idx;",
                "      idvd_excess = 0.0;",
                "    end",
                "",
                "    if(vd_mag <= 0.02) begin",
                "      idvd_vd0_idx = 0;",
                "      idvd_vd1_idx = 1;",
                "      idvd_vd_frac = 0.0;",
                "    end else if(vd_mag >= 18.0) begin",
                "      idvd_vd0_idx = 298;",
                "      idvd_vd1_idx = 299;",
                "      idvd_vd_frac = 1.0;",
                "    end else begin",
                "      idvd_vd_pos = (vd_mag - 0.02) / (17.98 / 299.0);",
                "      idvd_vd0_idx = $rtoi(idvd_vd_pos);",
                "      if(idvd_vd0_idx > 298) idvd_vd0_idx = 298;",
                "      idvd_vd1_idx = idvd_vd0_idx + 1;",
                "      idvd_vd_frac = idvd_vd_pos - idvd_vd0_idx;",
                "    end",
                "",
                "    idvd_i00 = idvd_out[idvd_vg0_idx * 300 + idvd_vd0_idx];",
                "    idvd_i01 = idvd_out[idvd_vg0_idx * 300 + idvd_vd1_idx];",
                "    idvd_i10 = idvd_out[idvd_vg1_idx * 300 + idvd_vd0_idx];",
                "    idvd_i11 = idvd_out[idvd_vg1_idx * 300 + idvd_vd1_idx];",
                ...
                "",
                "    // Id-Vg surface: 2 Vd anchors x 446 Vg samples",
                "    if(val_vg <= -1.9) begin",
                "      idvg_vg0_idx = 0;",
                "      idvg_vg1_idx = 1;",
                "      idvg_vg_frac = 0.0;",
                "    end else if(val_vg >= 7.0) begin",
                "      idvg_vg0_idx = 444;",
                "      idvg_vg1_idx = 445;",
                "      idvg_vg_frac = 1.0;",
                "    end else begin",
                "      idvg_vg_pos = (val_vg + 1.9) / 0.02;",
                "      idvg_vg0_idx = $rtoi(idvg_vg_pos);",
                "      if(idvg_vg0_idx > 444) idvg_vg0_idx = 444;",
                ...
                "    end else if(vd_mag >= 15.0) begin",
                "      idvg_vd0_idx = 0;",
                "      idvg_vd1_idx = 1;",
                "      idvg_vd_frac = 1.0;",
                "      idvg_excess = vd_mag - 15.0;",
                "    end else begin",
                "      idvg_vd0_idx = 0;",
                "      idvg_vd1_idx = 1;",
                "      idvg_vd_frac = (vd_mag - 1.0) / 14.0;",
                "      idvg_excess = 0.0;",
                "    end",
                "",
                "    idvg_i00 = idvg_out[idvg_vd0_idx * 446 + idvg_vg0_idx];",
                "    idvg_i01 = idvg_out[idvg_vd0_idx * 446 + idvg_vg1_idx];",
                "    idvg_i10 = idvg_out[idvg_vd1_idx * 446 + idvg_vg0_idx];",
                "    idvg_i11 = idvg_out[idvg_vd1_idx * 446 + idvg_vg1_idx];",
